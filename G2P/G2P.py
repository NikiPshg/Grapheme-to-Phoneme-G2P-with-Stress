...
from config_models import config_g2p
...
# Initialization
tokenizer_g2p = Tokenizer.from_file('D:/python/G2P_en_lexicon/my_tokenizer/bpe_512_lex.json')
g2p_model = TransformerBlock(config=config_g2p, tokenizer=tokenizer_g2p)
g2p_model.load_state_dict(torch.load('D:/python/G2P_en_lexicon/models/model_0.8.pt', map_location=torch.device('cpu')))

G2P = GraphemeToPhoneme(g2p_model, tokenizer_g2p)

if __name__ == '__main__':
    print(G2P('POOPA'))  # Expected output: ['P', 'UW', 'P', 'AH']
